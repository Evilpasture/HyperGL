    ...
    def __init__(self):
        ...
        if sys.platform.startswith('win'):
            ...
            try:
                ...
                self.load_opengl_function = lambda name: proc(name.encode())
                return
            except:
                ...
                self.load_opengl_function = lambda name: proc(name.encode()) or funcptr(lib, name)
                return

        elif sys.platform.startswith('darwin'):
            lib = ctypes.CDLL('/System/Library/Frameworks/OpenGL.framework/OpenGL')
            self.load_opengl_function = lambda name: funcptr(lib, name)
            return

        elif sys.platform.startswith('emscripten') or sys.platform.startswith('wasi'):
...
def headless_context_windows():
    ...
    pfd.nVersion = 1
    pfd.dwFlags = 37 # DRAW_TO_WINDOW | SUPPORT_OPENGL | DOUBLEBUFFER
    pfd.iPixelType = 0
    pfd.cColorBits = 32
    pfd.cDepthBits = 24
    pfd.cStencilBits = 8
    ...
    pixel_format_modern = c_int()
    num_formats = UINT()
    
    wglChoosePixelFormatARB(hdc, attribs, None, 1, byref(pixel_format_modern), byref(num_formats))
    gdi32.SetPixelFormat(hdc, pixel_format_modern.value, byref(pfd))
...
def web_context():
    ...
    callback = js.window.eval(hypergl._extern_gl)
...
def loader(headless=False):
    ...
    if sys.platform.startswith('win') and headless:
        import ctypes
        opengl32 = ctypes.windll.opengl32
        opengl32.wglGetProcAddress.restype = ctypes.c_void_p
        
        def win_loader(name):
            ptr = opengl32.wglGetProcAddress(name.encode())
            if not ptr:
                ptr = ctypes.cast(getattr(opengl32, name, 0), ctypes.c_void_p).value
            return ptr or 0
        loader.load_opengl_function = win_loader
