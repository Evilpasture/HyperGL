...
class CommandBuffer:
    ...
    def gen_rand(self, reg: int) -> None:
        """
        Bytecode Instruction: GEN_RAND
        Generates a pseudo-random 32-bit unsigned integer using Xorshift32 
        and stores it in i[reg].
        """
        ...

    def get_time(self, reg: int) -> None:
        """Stores float32 seconds since app start into i[reg]."""
        ...
    def get_delta(self, reg: int) -> None:
        """Stores float32 seconds since last submission into i[reg]."""
        ...
    ...
    def sin_cos(self, reg_in: int, dest_sin: int, dest_cos: int) -> None:
        """
        Bytecode Instruction: SIN_COS
        Treats i[reg_in] as float radians. Calculates sin/cos and stores 
        as float32 bit-patterns in i[dest_sin] and i[dest_cos].
        """
        ...

    def set_buffer_offset(self, buffer: Buffer, slot: int, reg: int, size: int, type: Literal['uniform', 'storage'] = 'uniform') -> None:
        """
        Bytecode Instruction: SET_BUFFER_OFFSET
        Dynamically binds a range of 'buffer' to hardware 'slot'.
        The start of the range is read from VM register i[reg].
        """
        ...
    ...
    def copy_buffer(self, src: Buffer, dst: Buffer, src_reg: int, dst_reg: int, size_reg: int) -> None:
        """
        Bytecode Instruction: COPY_BUFFER
        Performs a glCopyBufferSubData between 'src' and 'dst'.
        Offsets and size are read from the specified VM registers.
        """
        ...
